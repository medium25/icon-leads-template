...
/** hex (#RRGGBB) → строка «R G B» для CSS-переменной --lead-hdr-rgb (см. .lead-hdr в index.css). */
function hexTriplet(hex) {
  const h = (hex || '').replace('#', '');
  if (h.length !== 6) return '139 148 163';
...
export function LeadCard({
  ...
  return (
    ...
    >
      {/* Шапка карточки — в светлой теме бледная заливка цветом текущей
          колонки (column.color), в тёмной — нейтральный тёмно-серый (см.
          .lead-hdr в index.css). Отрицательные margin/rounded-t растягивают
          заливку до самых краёв карточки поверх её собственного p-3.5. */}
      <div
        className="lead-hdr -mx-3.5 -mt-3.5 flex items-center justify-between gap-2 rounded-t-xl border-b px-3.5 pb-2.5 pt-3.5"
        style={{ '--lead-hdr-rgb': hexTriplet(currentColumn?.color) }}
      >
        <div className="flex min-w-0 items-center gap-1.5">
          ...
          {createdAt && formatOverdueBy(createdAt) && (
            <span
              className="shrink-0 rounded-badge bg-black/[0.06] px-1.5 py-0.5 text-[10px] font-bold leading-none text-muted dark:bg-white/[0.09]"
        ...
        <div className="flex min-w-0 shrink items-center gap-1">
          {lead.vacancyName && <span className="truncate text-[12px] font-bold text-muted">{lead.vacancyName}</span>}
        </div>
      </div>
